...
public sealed class OrchestratorService
{
    ...
    public InstallationManifest RegisterInstance(string channel, int port, string folderName, string database, bool autoUpdate, string configPath)
    {
        ...
        var manifest = new InstallationManifest(
            id,
            channel == "Test" ? "iMonitor ERP Test" : "iMonitor ERP Production",
            channel,
            port,
            folderName,
    ...
    private void DiscoverLegacyInstances(IDictionary<string, InstallationManifest> manifests)
    {
        foreach (var channel in new[] { "Test", "Production" })
        {
            ...
            manifests[id] = new InstallationManifest(id, channel == "Test" ? "iMonitor ERP Test" : "iMonitor ERP Production", channel,
                channel == "Test" ? 8082 : 8083, folder, root, cfg, channel == "Test" ? "imonitor_test" : "imonitor",
                channel == "Test" ? "iMonitorERP-Test" : "iMonitorERP-Production", channel == "Test" ? "iMonitorERP-Test" : "iMonitorERP-Production",
                ScheduledTaskExists(channel == "Test" ? "iMonitorERP-Update-Test" : "iMonitorERP-Update-Production"), DateTime.UtcNow, DateTime.UtcNow);
        }
    }
    ...
    private async Task<Dictionary<string, string>> GetLatestReleasesAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            http.DefaultRequestHeaders.UserAgent.ParseAdd("iMonitorERP-Orchestrator/1.0");
            ...
            foreach (var e in doc.RootElement.EnumerateArray())
            {
                ...
                if (tag.StartsWith("imonitor-erp-test-v", StringComparison.OrdinalIgnoreCase) && !result.ContainsKey("Test")) result["Test"] = tag;
                if (tag.StartsWith("imonitor-erp-production-v", StringComparison.OrdinalIgnoreCase) && !result.ContainsKey("Production")) result["Production"] = tag;
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), "imonitor-mysql-" + Guid.NewGuid().ToString("N") + ".cnf");
