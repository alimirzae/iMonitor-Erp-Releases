...
$stableInstaller=Join-Path $stableDir 'Install-PosiranERP-v1.0.4.ps1'
...
function Invoke-HttpDownload([string]$Url,[string]$Out,[string]$Accept='application/octet-stream',[int]$TimeoutSeconds=600){
  ...
  $client.DefaultRequestHeaders.UserAgent.ParseAdd('PosiranERP-Installer/1.0.4')
...
function Invoke-AssetDownload([string]$ApiUrl,[string]$BrowserUrl,[string]$Out,[int]$TimeoutSeconds=600){
  ...
  try{
    ...
    Invoke-WebRequest -UseBasicParsing -Uri $BrowserUrl -OutFile $Out -TimeoutSec $TimeoutSeconds -Headers @{'User-Agent'='PosiranERP-Installer/1.0.4';'Cache-Control'='no-cache'}
    if((Test-Path $Out) -and (Get-Item $Out).Length -gt 0){return}
...
function Get-LatestRelease($info){
  ...
  $headers=@{'User-Agent'='PosiranERP-Installer/1.0.4';'Accept'='application/vnd.github+json';'Cache-Control'='no-cache'}
...
param([switch]`$Force)
`$ErrorActionPreference='Stop'
& $(Q $stableInstaller) -Channel $(Q $info.Name) -Mode UpdateOnly -InstallRoot $(Q $InstallRoot) -ConfigRoot $(Q $ConfigRoot) -TestFolderName $(Q $TestFolderName) -ProductionFolderName $(Q $ProductionFolderName) -TestPort $TestPort -ProductionPort $ProductionPort -SkipTaskRegistration -Force:`$Force
"@
...
function Install-Channel($info){
  ...
  try{
    ...
    Expand-Archive $zip -DestinationPath $info.Root -Force;Copy-Item $info.Config (Join-Path $info.Root 'appsettings.json') -Force;Write-LocalUpdater $info $info.Root
