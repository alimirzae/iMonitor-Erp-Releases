...
Write-Host '=== iMonitor ERP Windows installer v2.0.19 ===' -ForegroundColor Cyan
Write-Host 'Installer revision: 2.0.19-r1 (independent Test/Production)' -ForegroundColor DarkCyan
...
Write-Host 'Databases    : Test=ecomm_dev, Production=ecomm'
Write-Host 'Migration    : deferred/disabled during installer run'
Write-Host 'Policy       : Test success is preserved if Production fails; Production updater retries every 5 minutes.' -ForegroundColor Cyan
$self=Join-Path $installerHome 'Install-iMonitorERP-v2.0.19.ps1'
Copy-Item $PSCommandPath $self -Force
foreach($old in @('Install-iMonitorERP-v2.0.15.ps1','Install-iMonitorERP-v2.0.16.ps1','Install-iMonitorERP-v2.0.17.ps1','Install-iMonitorERP-v2.0.18.ps1')){
  $p=Join-Path $installerHome $old
  if(Test-Path $p -PathType Leaf){Remove-Item $p -Force -ErrorAction SilentlyContinue}
}
function Register-Updaters {
    ...
    Write-Host '[OK] Automatic updater tasks now point to v2.0.19 (every 5 minutes).' -ForegroundColor Green
}
Register-Updaters
...
$core=Join-Path $env:TEMP ('Install-iMonitorERP-v2.0.19-core-'+[guid]::NewGuid().ToString('N')+'.ps1')
$cb=[DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds()
$uri="https://raw.githubusercontent.com/$repo/main/scripts/Install-iMonitorERP-v2.0.19-core.ps1?cb=$cb"
Write-Host 'Downloading installer core v2.0.19 over IPv4...'
...
Write-Host 'iMonitor ERP v2.0.19 completed.' -ForegroundColor Green
