...
$stableInstaller=Join-Path $stableDir 'Install-iMonitorERP-v2.1.3.ps1'
...
function Invoke-HttpDownload([string]$Url,[string]$Out,[string]$Accept='application/octet-stream',[int]$TimeoutSeconds=600){
  ...
  $client.DefaultRequestHeaders.UserAgent.ParseAdd('iMonitorERP-Installer/2.1.3')
...
function Invoke-AssetDownload([string]$ApiUrl,[string]$BrowserUrl,[string]$Out,[int]$TimeoutSeconds=600){
  ...
  try{
    ...
    Invoke-WebRequest -UseBasicParsing -Uri $BrowserUrl -OutFile $Out -TimeoutSec $TimeoutSeconds -Headers @{'User-Agent'='iMonitorERP-Installer/2.1.3';'Cache-Control'='no-cache'}
    if((Test-Path $Out) -and (Get-Item $Out).Length -gt 0){return}
...
function Get-LatestRelease($info){
  ...
  $uri="https://api.github.com/repos/$repo/releases?per_page=100&cb=$cb";$headers=@{'User-Agent'='iMonitorERP-Installer/2.1.3';'Accept'='application/vnd.github+json';'Cache-Control'='no-cache'}
...
function Register-Updater($info){
  ...
  $localUpdater=Join-Path $info.Root 'Update-iMonitorERP.ps1'
