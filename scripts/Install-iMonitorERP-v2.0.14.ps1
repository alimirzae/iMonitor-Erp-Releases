...
Write-Host 'iMonitor ERP Windows installer v2.0.14' -ForegroundColor Cyan
Write-Host "Install root : $InstallRoot"
Write-Host "Package cache: $PackageCacheDirectory"
Write-Host "Channel      : $Channel"

# Persist this bootstrap first so automatic updates exist even if first activation fails.
$self = Join-Path $installerHome 'Install-iMonitorERP-v2.0.14.ps1'
Copy-Item $PSCommandPath $self -Force

function Register-Updaters {
    $testAction = "powershell.exe -NoProfile -ExecutionPolicy Bypass -File `"$self`" -Channel Test -InstallRoot `"$InstallRoot`" -PackageCacheDirectory `"$PackageCacheDirectory`" -TestPort $TestPort -ProductionPort $ProductionPort -ManagedMySqlPort $ManagedMySqlPort -UpdateOnly"
    $prodAction = "powershell.exe -NoProfile -ExecutionPolicy Bypass -File `"$self`" -Channel Production -InstallRoot `"$InstallRoot`" -PackageCacheDirectory `"$PackageCacheDirectory`" -TestPort $TestPort -ProductionPort $ProductionPort -ManagedMySqlPort $ManagedMySqlPort -UpdateOnly"
...
# Refresh PATH so a Hosting Bundle or other dependency installed in a previous run is visible.
...
$core=Join-Path $env:TEMP 'Install-iMonitorERP-v2.0.14-core.ps1'
$cb=[DateTimeOffset]::UtcNow.ToUnixTimeSeconds()
$uri="https://raw.githubusercontent.com/$repo/main/scripts/Install-iMonitorERP-v2.0.14-core.ps1?cb=$cb"

Write-Host 'Downloading installer core v2.0.14 over IPv4...'
...
$args=@('-NoProfile','-ExecutionPolicy','Bypass','-File',$core,'-Channel',$Channel,'-InstallRoot',$InstallRoot,'-PackageCacheDirectory',$PackageCacheDirectory,'-TestPort',[string]$TestPort,'-ProductionPort',[string]$ProductionPort,'-ManagedMySqlPort',[string]$ManagedMySqlPort)
if($Force){$args+='-Force'}
...
try {
    ...
    # Core v2.0.13 may recreate legacy tasks; always restore v2.0.14 tasks last.
    Register-Updaters
}

Write-Host 'iMonitor ERP v2.0.14 completed.' -ForegroundColor Green
