...
curl -fsSL --retry 5 --retry-delay 2 "$MANIFEST_URL" -o "$TMP"
...
echo "[iMonitor updater] manifest installer v${VER}: ${PATHNAME}"
curl -fsSL --retry 5 --retry-delay 2 "https://raw.githubusercontent.com/${REPO}/main/${PATHNAME}?cachebust=$(date +%s)" -o "$INSTALLER.tmp"
