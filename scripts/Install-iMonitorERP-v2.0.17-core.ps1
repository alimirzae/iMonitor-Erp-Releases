...
$patched=Join-Path $env:TEMP ('imonitor-core-v2017-'+[guid]::NewGuid().ToString('N')+'.ps1')
...
try {
  ...
  Write-Host 'Loading base installer logic for v2.0.17...' -ForegroundColor Cyan
  ...
  $newCurl="            & `$curl.Source -4 --http1.1 --silent --show-error --fail --location --connect-timeout 10 --max-time `$MaxTime --retry 2 --retry-all-errors -H 'User-Agent: iMonitorERP-Installer/2.0.17' `$Uri -o `$OutFile 2>`$null`r`n            `$code = `$LASTEXITCODE"
...
function Get-LatestRelease([string]$Prefix) {
    ...
    try {
        ...
        return [pscustomobject]@{
            tag_name = $tagName
            assets = @([pscustomobject]@{ name = $assetName; browser_download_url = $downloadUrl })
        }
    } finally { Remove-Item $tmp -Force -ErrorAction SilentlyContinue }
}
'@
...
'@
  ...
  $text=$text.Replace("Write-Host 'iMonitor ERP installer core v2.0.15' -ForegroundColor Cyan","Write-Host 'iMonitor ERP installer core v2.0.17' -ForegroundColor Cyan")
