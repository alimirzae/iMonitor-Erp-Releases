...
$self=Join-Path $installerHome 'Install-iMonitorERP-v2.0.24.ps1'
Copy-Item $PSCommandPath $self -Force
foreach($old in @('Install-iMonitorERP-v2.0.15.ps1','Install-iMonitorERP-v2.0.16.ps1','Install-iMonitorERP-v2.0.17.ps1','Install-iMonitorERP-v2.0.18.ps1','Install-iMonitorERP-v2.0.19.ps1','Install-iMonitorERP-v2.0.20.ps1','Install-iMonitorERP-v2.0.21.ps1','Install-iMonitorERP-v2.0.22.ps1','Install-iMonitorERP-v2.0.23.ps1')){ $p=Join-Path $installerHome $old; if(Test-Path $p){Remove-Item $p -Force -ErrorAction SilentlyContinue} }
...
$core=Join-Path $env:TEMP ('Install-iMonitorERP-v2.0.24-core-'+[guid]::NewGuid().ToString('N')+'.ps1')
$cb=[DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds()
$uri="https://raw.githubusercontent.com/$repo/main/scripts/Install-iMonitorERP-v2.0.24-core.ps1?cb=$cb"
Write-Host 'Downloading installer core v2.0.24 over IPv4...'
