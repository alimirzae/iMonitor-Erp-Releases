...
function Ensure-Site([string]$Name,[string]$Pool,[string]$PhysicalPath,[int]$Port,[string]$HostName){
    Ensure-AppPool $Pool
    if(Get-Website -Name $Name -ErrorAction SilentlyContinue){ Remove-Website -Name $Name }
    New-Website -Name $Name -PhysicalPath $PhysicalPath -Port $Port -HostHeader $HostName -ApplicationPool $Pool | Out-Null
    ...
    & icacls $PhysicalPath /grant "${identity}:(OI)(CI)(RX)" /T /C | Out-Null
    foreach($rel in @('logs','data','App_Data','temp','uploads','wwwroot\uploads')){
        $wp=Join-Path $PhysicalPath $rel; New-Item -ItemType Directory -Force $wp | Out-Null
