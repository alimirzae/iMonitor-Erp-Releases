...
$stableInstaller=Join-Path $stableDir 'Install-iMonitorERP-v2.1.0.ps1'
...
function Invoke-HttpDownload([string]$Url,[string]$Out,[string]$Accept='application/octet-stream',[int]$TimeoutSeconds=600){
  ...
  $client.DefaultRequestHeaders.UserAgent.ParseAdd('iMonitorERP-Installer/2.1.0')
...
function Invoke-AssetDownload([string]$ApiUrl,[string]$BrowserUrl,[string]$Out,[int]$TimeoutSeconds=600){
  ...
  try{
    ...
    Invoke-WebRequest -UseBasicParsing -Uri $BrowserUrl -OutFile $Out -TimeoutSec $TimeoutSeconds -Headers @{'User-Agent'='iMonitorERP-Installer/2.1.0';'Cache-Control'='no-cache'}
    if((Test-Path $Out) -and (Get-Item $Out).Length -gt 0){return}
