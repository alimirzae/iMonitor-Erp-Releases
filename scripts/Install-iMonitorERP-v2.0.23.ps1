...
Write-Host '=== iMonitor ERP Windows installer v2.0.23 ===' -ForegroundColor Cyan
Write-Host 'Installer revision: 2.0.23-r1 (1-minute Test auto-updater)' -ForegroundColor DarkCyan
...
$self=Join-Path $installerHome 'Install-iMonitorERP-v2.0.23.ps1'
Copy-Item $PSCommandPath $self -Force
foreach($old in @('Install-iMonitorERP-v2.0.15.ps1','Install-iMonitorERP-v2.0.16.ps1','Install-iMonitorERP-v2.0.17.ps1','Install-iMonitorERP-v2.0.18.ps1','Install-iMonitorERP-v2.0.19.ps1','Install-iMonitorERP-v2.0.20.ps1','Install-iMonitorERP-v2.0.21.ps1','Install-iMonitorERP-v2.0.22.ps1')){ $p=Join-Path $installerHome $old; if(Test-Path $p){Remove-Item $p -Force -ErrorAction SilentlyContinue} }
function Register-Updaters {
  ...
  & schtasks.exe /Create /F /TN 'iMonitorERP-Update-Test' /SC MINUTE /MO 1 /RU SYSTEM /RL HIGHEST /TR $testAction | Out-Null
  & schtasks.exe /Create /F /TN 'iMonitorERP-Update-Production' /SC MINUTE /MO 5 /RU SYSTEM /RL HIGHEST /TR $prodAction | Out-Null
  $global:LASTEXITCODE=0
  Write-Host '[OK] Test updater: every 1 minute. Production updater: every 5 minutes.' -ForegroundColor Green
}
Register-Updaters
$repo='alimirzae/iMonitor-Erp-Releases'
$core=Join-Path $env:TEMP ('Install-iMonitorERP-v2.0.23-core-'+[guid]::NewGuid().ToString('N')+'.ps1')
$cb=[DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds()
$uri="https://raw.githubusercontent.com/$repo/main/scripts/Install-iMonitorERP-v2.0.23-core.ps1?cb=$cb"
Write-Host 'Downloading installer core v2.0.23 over IPv4...'
...
Write-Host 'iMonitor ERP v2.0.23 completed.' -ForegroundColor Green
