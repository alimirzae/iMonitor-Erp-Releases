...
$stableInstaller=Join-Path $stableDir 'Install-PosiranERP-v1.0.2.ps1'
...
function Invoke-Curl([string]$Url,[string]$Out,[int]$MaxTime=600){
  & curl.exe -4 --http1.1 --silent --show-error --fail --location --connect-timeout 8 --max-time $MaxTime --retry 4 --retry-all-errors -H 'Cache-Control: no-cache' -H 'Pragma: no-cache' -H 'User-Agent: PosiranERP-Installer/1.0.2' $Url -o $Out
...
function Normalize-ChannelConfig($info){
  ...
  if($cs -match '(?i)(Database|Initial Catalog)\s*='){
    $cs=[regex]::Replace($cs,'(?i)(Database|Initial Catalog)\s*=\s*[^;]*',"Database=$($info.Database)")
  }else{
    $cs=$cs.TrimEnd(';')+";Database=$($info.Database);"
  }
...
function Install-Channel($info){
  ...
  try{
    ...
    Set-Content $info.State $rel.Tag -Encoding ASCII;Write-Host "[OK] $($rel.Tag) -> http://127.0.0.1:$($info.Port)/ ; DB=$($info.Database)" -ForegroundColor Green;Register-Updater $info
