...
if (( ${#missing_tools[@]} > 0 )); then
  log "Missing tools: ${missing_tools[*]}; attempting installation from configured Ubuntu repositories"
  apt-get -o Acquire::ForceIPv4=true update || log "WARNING: apt index refresh failed; trying cached package indexes"
  apt-get -o Acquire::ForceIPv4=true install -y --no-install-recommends curl jq ca-certificates tar coreutils \
    || fail "Required tools are missing and APT repositories are unreachable. Fix server routing/DNS, then retry."
else
  log "Base tools already installed; skipping apt-get update"
fi
...
if [[ -n "$SCRIPT_SOURCE" && -f "$SCRIPT_SOURCE" && "$SCRIPT_SOURCE" != "$INSTALLER_PATH" ]]; then
  ...
  curl -fsSL --retry 5 \
    "https://raw.githubusercontent.com/${REPO}/main/scripts/Install-iMonitorEcomERP-v1.0.6.sh" \
    -o "$INSTALLER_PATH"
  chmod 0755 "$INSTALLER_PATH"
fi
release_json="$(curl -fsSL --retry 8 --retry-all-errors --connect-timeout 15 "${API}?per_page=100&cachebust=$(date +%s)")"
if [[ "$CHANNEL" == "test" ]]; then
  # Test builds are intentionally marked as GitHub prereleases.
  tag="$(jq -r --arg prefix "imonitor-ecomerp-test-v" '
    [.[] | select(.draft == false) | select(.tag_name | startswith($prefix))]
    | sort_by(.published_at // .created_at) | last | .tag_name // empty
  ' <<<"$release_json")"
else
  tag="$(jq -r --arg prefix "imonitor-ecomerp-master-v" '
    [.[] | select(.draft == false and .prerelease == false) | select(.tag_name | startswith($prefix))]
    | sort_by(.published_at // .created_at) | last | .tag_name // empty
  ' <<<"$release_json")"
fi
if [[ -z "$tag" ]]; then
  echo "Available ERP release tags:" >&2
  jq -r '.[] | select(.draft == false) | .tag_name' <<<"$release_json" | grep '^imonitor-ecomerp-' >&2 || true
  fail "No release found for channel $CHANNEL"
fi
...
log "Installer 1.0.6 completed; channel=$CHANNEL port=$PORT tag=$tag"
