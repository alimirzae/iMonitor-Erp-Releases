...
# v2.0.4 rule: explicit CLI value > persisted credential file > currently installed appsettings > built-in default.
...
# v2.0.2 creates tasks pointing back to itself. Replace them with v2.0.4.
# Passwords are intentionally NOT placed in Task Scheduler command lines; v2.0.4 reloads them from the protected credential file.
$self = Join-Path $installerHome 'Install-iMonitorERP-v2.0.4.ps1'
Invoke-WebRequest -UseBasicParsing "https://raw.githubusercontent.com/$repo/main/scripts/Install-iMonitorERP-v2.0.4.ps1?cb=$([DateTimeOffset]::UtcNow.ToUnixTimeSeconds())" -OutFile $self
...
Write-Host ''
Write-Host 'iMonitor ERP installer v2.0.4 completed.'
