...
CACHE_ROOT="/var/cache/imonitor-platform/downloads"
...
install_base_deps
mkdir -p "$PRODUCT_ROOT/releases" "$STATE_ROOT/installers" "$CONFIG_ROOT" "$CACHE_ROOT"
...
log "Downloading release $VERSION (persistent cache with resume enabled)"
curl -fL --retry 8 --retry-delay 3 --continue-at - "$SRC_URL" -o "$CACHE_ROOT/$SRC_NAME"
curl -fL --retry 8 --retry-delay 3 --continue-at - "$IMG_URL" -o "$CACHE_ROOT/$IMG_NAME"
curl -fL --retry 8 --retry-delay 3 "$SUM_URL" -o "$TMP/$SUM_NAME"
grep -F -e "  $SRC_NAME" -e "  $IMG_NAME" "$TMP/$SUM_NAME" > "$TMP/required-sha256.txt"
[[ "$(wc -l < "$TMP/required-sha256.txt")" -eq 2 ]] || { echo "Release checksum file does not contain both required assets." >&2; exit 4; }
if ! (cd "$CACHE_ROOT" && sha256sum -c "$TMP/required-sha256.txt"); then
  rm -f "$CACHE_ROOT/$SRC_NAME" "$CACHE_ROOT/$IMG_NAME"
  echo "Downloaded release failed checksum validation; cached files were removed. Run the installer again." >&2
  exit 4
fi
RELEASE_DIR="$PRODUCT_ROOT/releases/$VERSION"
rm -rf "$RELEASE_DIR"; mkdir -p "$RELEASE_DIR"
tar -xzf "$CACHE_ROOT/$SRC_NAME" -C "$RELEASE_DIR" --strip-components=1
...
if ! gzip -dc "$CACHE_ROOT/$IMG_NAME" | docker load; then
...
if [[ $UPDATE_ONLY -eq 0 ]]; then
  UPDATER_URL="https://raw.githubusercontent.com/${RELEASE_REPO}/main/scripts/Update-iMonitorPlatform-v1.0.3.sh?cachebust=$(date +%s)"
