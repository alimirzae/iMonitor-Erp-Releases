...
Write-Host '=== iMonitor ERP Windows installer v2.0.18 ===' -ForegroundColor Cyan
Write-Host 'Installer revision: 2.0.18-r1' -ForegroundColor DarkCyan
...
$self=Join-Path $installerHome 'Install-iMonitorERP-v2.0.18.ps1'
Copy-Item $PSCommandPath $self -Force

foreach($old in @('Install-iMonitorERP-v2.0.15.ps1','Install-iMonitorERP-v2.0.16.ps1','Install-iMonitorERP-v2.0.17.ps1')){
...
function Register-Updaters {
    ...
    Write-Host '[OK] Automatic updater tasks now point to v2.0.18 (every 5 minutes).' -ForegroundColor Green
}
...
$core=Join-Path $env:TEMP ('Install-iMonitorERP-v2.0.18-core-'+[guid]::NewGuid().ToString('N')+'.ps1')
$cb=[DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds()
$uri="https://raw.githubusercontent.com/$repo/main/scripts/Install-iMonitorERP-v2.0.18-core.ps1?cb=$cb"
Write-Host 'Downloading installer core v2.0.18 over IPv4...'
...
Write-Host 'iMonitor ERP v2.0.18 completed.' -ForegroundColor Green
