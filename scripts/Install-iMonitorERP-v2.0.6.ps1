...
function Test-UrlLatency([string]$Uri,[bool]$ForceIpv4=$false) {
    try {
        ...
        if($curl){
            $args=@('--silent','--fail','--location','--head','--connect-timeout','3','--max-time','6')
            if($ForceIpv4){$args=@('-4')+$args}
            & $curl.Source @args $Uri 2>$null | Out-Null
            if($LASTEXITCODE -ne 0){$global:LASTEXITCODE=0; return [double]::PositiveInfinity}
        } else {
...
function Get-PreferredSource {
    ...
    $mirrorText=if([double]::IsPositiveInfinity($mirrorMs)){'unavailable'}else{"$([math]::Round($mirrorMs)) ms"}
    $githubText=if([double]::IsPositiveInfinity($githubMs)){'unavailable'}else{"$([math]::Round($githubMs)) ms"}
    Write-Host "Download source probe: mirror=$mirrorText, github-ipv4=$githubText"
    if([double]::IsPositiveInfinity($mirrorMs) -and [double]::IsPositiveInfinity($githubMs)){
        if(Test-Path $sourcePreferenceFile){
            $savedSource=(Get-Content $sourcePreferenceFile -Raw).Trim()
            if($savedSource -in @('mirror','github')){return $savedSource}
        }
        return 'github'
    }
    ...
    foreach($source in $sources){
        ...
        try {
            ...
            if($curl){
                ...
                if($curlExit -ne 0){throw "curl exit code $curlExit"}
            } else {
...
function Get-RuntimeConfigValue([string]$ChannelName,[string]$PropertyName) {
    ...
    try {
        ...
        $database = Get-SavedProperty $settings 'Database'
        $mysql = Get-SavedProperty $database 'MySql'
        if ($null -eq $mysql) { return $null }

        switch ($PropertyName) {
            'Server' { return (Get-SavedProperty $mysql 'Server') }
            'Port' { return (Get-SavedProperty $mysql 'Port') }
            'User' {
                return (First-NonEmpty @(
                    (Get-SavedProperty $mysql 'UserId'),
                    (Get-SavedProperty $mysql 'User'),
                    (Get-SavedProperty $mysql 'Username')
                ))
            }
            'Database' {
                $connectionStrings = Get-SavedProperty $settings 'ConnectionStrings'
                $cs = [string](First-NonEmpty @(
                    (Get-SavedProperty $mysql 'ConnectionString'),
                    (Get-SavedProperty $connectionStrings 'MySql'),
                    (Get-SavedProperty $connectionStrings 'DefaultConnection')
                ))
                if ($cs -match '(?i)(?:^|;)\s*(?:Database|Initial Catalog)\s*=\s*([^;]+)') { return $matches[1].Trim() }
                return (First-NonEmpty @(
                    (Get-SavedProperty $mysql 'Database'),
                    (Get-SavedProperty $mysql 'DatabaseName')
                ))
            }
        }
    } catch {
...
# v2.0.2 remains the verified compatibility/install engine. v2.0.6 owns configuration,
# source selection, updater persistence and scheduling.
...
Invoke-SourceDownload `
    ...
    $baseInstaller 'Compatibility installer download'
...
$self = Join-Path $installerHome 'Install-iMonitorERP-v2.0.6.ps1'
Invoke-SourceDownload `
    "$mirrorRoot/install/Install-iMonitorERP-v2.0.6.ps1.txt?cb=$([DateTimeOffset]::UtcNow.ToUnixTimeSeconds())" `
    "https://raw.githubusercontent.com/$repo/main/scripts/Install-iMonitorERP-v2.0.6.ps1?cb=$([DateTimeOffset]::UtcNow.ToUnixTimeSeconds())" `
...
Write-Host ''
Write-Host 'iMonitor ERP installer/updater v2.0.6 completed.'
