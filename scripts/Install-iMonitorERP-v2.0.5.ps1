...
# Self-refresh the versioned updater and repoint scheduled tasks to it on every run.
# Passwords are never placed on task command lines; v2.0.5 reloads them from the protected credential file.
$self = Join-Path $installerHome 'Install-iMonitorERP-v2.0.5.ps1'
Invoke-WebRequest -UseBasicParsing "https://raw.githubusercontent.com/$repo/main/scripts/Install-iMonitorERP-v2.0.5.ps1?cb=$([DateTimeOffset]::UtcNow.ToUnixTimeSeconds())" -OutFile $self
...
Write-Host 'Every scheduled run refreshes this updater before recreating the schedules.'
