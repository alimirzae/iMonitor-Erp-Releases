...
$stableInstaller=Join-Path $stableDir 'Install-iMonitorERP-v2.1.2.ps1'
...
function Get-ChannelInfo([string]$Name){
  ...
  [pscustomobject]@{
    ...
    Task=if($isTest){'iMonitorERP-Update-Test'}else{'iMonitorERP-Update-Production'};Minutes=if($isTest){1}else{5};
...
function Invoke-HttpDownload([string]$Url,[string]$Out,[string]$Accept='application/octet-stream',[int]$TimeoutSeconds=600){
  ...
  $client.DefaultRequestHeaders.UserAgent.ParseAdd('iMonitorERP-Installer/2.1.2')
...
function Invoke-AssetDownload([string]$ApiUrl,[string]$BrowserUrl,[string]$Out,[int]$TimeoutSeconds=600){
  ...
  try{
    ...
    Invoke-WebRequest -UseBasicParsing -Uri $BrowserUrl -OutFile $Out -TimeoutSec $TimeoutSeconds -Headers @{'User-Agent'='iMonitorERP-Installer/2.1.2';'Cache-Control'='no-cache'}
    if((Test-Path $Out) -and (Get-Item $Out).Length -gt 0){return}
...
function Get-LatestRelease($info){
  ...
  $uri="https://api.github.com/repos/$repo/releases?per_page=100&cb=$cb";$headers=@{'User-Agent'='iMonitorERP-Installer/2.1.2';'Accept'='application/vnd.github+json';'Cache-Control'='no-cache'}
...
function Normalize-ChannelConfig($info){
  ...
  $update=[pscustomobject][ordered]@{Repository=$repo;Channel=$info.Key;TestTagPrefix='imonitor-ecomerp-test-v';ProductionTagPrefix='imonitor-ecomerp-master-v';TestTaskName='iMonitorERP-Update-Test';ProductionTaskName='iMonitorERP-Update-Production';TestIntervalMinutes=1;ProductionIntervalMinutes=5}
...
function Register-Updater($info){
  ...
  $args="-NoProfile -ExecutionPolicy Bypass -File `"$stableInstaller`" -Channel $($info.Name) -Mode UpdateOnly -InstallRoot `"$InstallRoot`" -ConfigRoot `"$ConfigRoot`" -TestFolderName `"$TestFolderName`" -ProductionFolderName `"$ProductionFolderName`" -TestPort $TestPort -ProductionPort $ProductionPort -TestHostHeader `"$TestHostHeader`" -ProductionHostHeader `"$ProductionHostHeader`" -TestPhysicalPath `"$TestPhysicalPath`" -ProductionPhysicalPath `"$ProductionPhysicalPath`" -SkipMySqlProvisioning -SkipTaskRegistration"
